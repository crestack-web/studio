...
function toRow(tableName: string, data: Record<string, unknown>): Record<string, unknown> {
  ...
  if (tableName === 'suppliers') {
    // Schema: id, business_id, name, phone, email, address, active, metadata, created_at, updated_at
    const displayName =
      data.supplierName ?? data.businessName ?? data.name ?? null;
    if (displayName != null && String(displayName).trim()) {
      row.name = String(displayName).trim();
    }
    if (row.active == null) {
      if (typeof data.active === 'boolean') row.active = data.active;
      else if (typeof data.status === 'string') row.active = data.status.toLowerCase() === 'active';
    ...
    const meta: Record<string, unknown> =
      row.metadata && typeof row.metadata === 'object' && !Array.isArray(row.metadata)
        ...
        : {};
    // Prefer businessName as company, supplierName as contact when both provided
    if (data.supplierName != null) meta.supplierName = data.supplierName;
    if (data.businessName != null) meta.businessName = data.businessName;
    for (const k of [
      'paymentTerms','customPaymentDays','creditLimit','openingBalance','currentBalance',
      'category','taxId','bankAccount','contactPerson','notes','lastPurchaseDate','lastPaymentDate',
      'totalPurchases','totalPayments','purchaseCount','paymentCount','averagePaymentDays','creditUtilization'
    ] as const) {
      if (data[k] !== undefined && data[k] !== null) meta[k] = data[k];
    }
    // Strip non-column leftovers that may have been assigned incorrectly
    delete row.status;
    delete row.balance;
    delete row.total_purchases;
    delete row.contact;
    delete row.rating;
    delete row.notes;
    delete row.supplier_name;
...
function toDoc(tableName: string, row: Record<string, unknown>): Record<string, unknown> {
  ...
  if (tableName === 'suppliers') {
    ...
    const displayName = (row.name as string) || (m.businessName as string) || (m.supplierName as string) || '';
    doc.name = displayName;
    doc.supplierName = (m.supplierName as string) || displayName;
    doc.businessName = (m.businessName as string) || displayName;
    doc.status = row.active === false ? 'inactive' : 'active';
    doc.active = row.active !== false;
    if (doc.currentBalance == null) doc.currentBalance = Number(m.currentBalance) || 0;
    if (doc.openingBalance == null) doc.openingBalance = Number(m.openingBalance) || 0;
    if (doc.creditLimit == null) doc.creditLimit = Number(m.creditLimit) || 0;
    if (doc.totalPurchases == null) doc.totalPurchases = Number(m.totalPurchases) || 0;
    if (doc.totalPayments == null) doc.totalPayments = Number(m.totalPayments) || 0;
    if (doc.purchaseCount == null) doc.purchaseCount = Number(m.purchaseCount) || 0;
    if (doc.paymentCount == null) doc.paymentCount = Number(m.paymentCount) || 0;
    if (doc.paymentTerms == null && m.paymentTerms != null) doc.paymentTerms = m.paymentTerms;
    if (doc.category == null) doc.category = (m.category as string) || 'general';
    if (doc.notes == null && m.notes != null) doc.notes = m.notes;
    if (doc.contactPerson == null && m.contactPerson != null) doc.contactPerson = m.contactPerson;
    if (doc.bankAccount == null && m.bankAccount != null) doc.bankAccount = m.bankAccount;
    if (doc.taxId == null && m.taxId != null) doc.taxId = m.taxId;
    if (doc.lastPurchaseDate == null && m.lastPurchaseDate != null) doc.lastPurchaseDate = m.lastPurchaseDate;
    if (doc.lastPaymentDate == null && m.lastPaymentDate != null) doc.lastPaymentDate = m.lastPaymentDate;
  }
  if (tableName === 'customers') {
    const m = (row.metadata && typeof row.metadata === 'object' && !Array.isArray(row.metadata))
      ? (row.metadata as Record<string, unknown>) : {};
