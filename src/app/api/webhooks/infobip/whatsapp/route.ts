 ...
 *
 * Phase 5A: merchant → existing MO path; platform → persist conversation/message + stub (no acquisition agent).
 */
...
const WEBHOOK_BUILD_MARKER = 'phase5a-platform-persist-v1';
...
/**
 * PHASE 5A TEMPORARY STUB — NOT the final Busmo Acquisition agent.
 * Message is persisted; AI agent lands in a later phase.
 */
const PHASE5A_PLATFORM_STUB_REPLY =
...
/**
 * PHASE 5A — platform path: persist conversation + claim inbound, then temporary stub.
 ...
 */
async function handlePlatformPhase5A(params: {
  ...
  console.log(
    JSON.stringify({
      ...
      phase: 'phase5a_platform_persist',
    })
  );
  ...
  // STOP before AI — no merchant MO / credits
  await updateMessageStatus(claim.id, 'received', {
    phase5a: true,
    stub: true,
    note: 'platform_message_persisted_awaiting_acquisition_agent',
  });

  console.log(
    JSON.stringify({
      event: 'phase5a_platform_stub',
      note: 'TEMPORARY — message persisted; acquisition agent not implemented',
      conversationId: conversation.id,
      claimId: claim.id,
      connectionId: resolved.connection.id,
      messageId,
  ...
  if (!isInfobipConfigured()) {
    console.error(
      JSON.stringify({
        event: 'phase5a_stub_send_skipped',
  ...
  const clientMessageId = `busmo-p5a-stub-${messageId}`.slice(0, 200);
  const sendResult = await sendWhatsAppText({
    from: businessSender,
    to: customerPhone,
    text: PHASE5A_PLATFORM_STUB_REPLY,
    messageId: clientMessageId,
  });

  console.log(
    JSON.stringify({
      event: sendResult.ok ? 'phase5a_stub_sent' : 'phase5a_stub_send_failed',
      messageId,
      clientMessageId,
