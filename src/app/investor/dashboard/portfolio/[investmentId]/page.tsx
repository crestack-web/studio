...
export default function InvestmentDetailsPage() {
    ...
    const renderDetails = () => {
        if (isLoadingInvestment || isLoadingBusiness) {
        ...
        return (
            <div className="grid md:grid-cols-2 gap-8">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4 text-sm">
                        {investment.type === 'Profit Sharing' && terms.profitShare && (
                            <DataPoint icon={Percent} label="Profit Share" value={`${terms.profitShare}%`} />
                        )}
                        {investment.type === 'Profit Sharing' && terms.duration && (
                             <DataPoint icon={Calendar} label="Duration" value={`${terms.duration} months`} />
                        )}
                        {investment.type === 'Equity' && terms.equity && (
                             <DataPoint icon={Percent} label="Equity Offered" value={`${terms.equity}%`} />
                        )}
                         {investment.type === 'Equity' && terms.valuation && (
                             <DataPoint icon={Banknote} label="Valuation" value={`${currency}${terms.valuation.toLocaleString()}`} />
                        )}
                    </CardContent>
                </Card>
