...
export default function BusmoOnboarding() {
  ...
  useEffect(() => {
    ...
    if (googleCallback === "callback" || authCode) {
      (async () => {
        try {
          ...
          if (session?.user) {
            ...
            setData((prev) => ({
              ...
              selectedFeatures: Array.isArray(prev.selectedFeatures)
                ? prev.selectedFeatures
                : CATEGORY_FEATURES["retail"],
              googleUserId: u.id,
            }));
            setIsGoogleAuth(true);
            setStep(2);
            const clean = `${window.location.pathname}?google=callback`;
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const supabase = getSupabase();
      const next = encodeURIComponent("/welcome/signup?google=callback");
