    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          setPermissions({
            sale: data.permissions?.sale ?? true,
            inv: data.permissions?.inv ?? true,
            hist: data.permissions?.hist ?? true,
            atd: data.permissions?.atd ?? true,
            msg: data.permissions?.msg ?? true,
            earn: data.permissions?.earn ?? true,
          });
        } else {
          router.push('/login/staff');
        }
