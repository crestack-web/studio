import React from 'react';
import type { PageId, StaffUser } from '../types';

const PAGE_META: Record<PageId, { title: string; sub: string }> = {
  home:       { title: 'Staff Dashboard',  sub: 'Your shift overview'                    },
  sale:       { title: 'Record a Sale',    sub: 'Select products and confirm the sale'   },
  inv:        { title: 'Inventory',        sub: 'View stock levels and alerts'           },
  hist:       { title: 'Sale History',     sub: 'All sales recorded during your shifts'  },
  atd:        { title: 'Shift & Attendance', sub: 'Track clock-ins and shift hours'      },
  msg:        { title: 'Messages',         sub: 'Chat with owner and team'               },
  settings:   { title: 'Settings',         sub: 'Personalise your portal experience'     },
};

interface TopbarProps {
  page: PageId;
  staff: StaffUser;
  chipTime: string;
  theme: 'light' | 'dark' | 'system';
  onToggleTheme: () => void;
  onOpenSidebar: () => void;
  onLogout?: () => void;
  hasNotification?: boolean;
}

export const Topbar: React.FC<TopbarProps> = ({
  page, staff, chipTime, theme, onToggleTheme, onOpenSidebar, onLogout, hasNotification = true,
}) => {
  const meta = PAGE_META[page];

  return (
    <header className="topbar">
      <button className="ham" onClick={onOpenSidebar} aria-label="Open menu">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <line x1="3" y1="12" x2="21" y2="12"/>
          <line x1="3" y1="6" x2="21" y2="6"/>
          <line x1="3" y1="18" x2="21" y2="18"/>
        </svg>
      </button>

      <div className="tb-ttl">
        <h1>{meta.title}</h1>
        <p>{meta.sub}</p>
      </div>

      <div className="tb-acts">
        {/* Shift chip */}
        <div className="shift-chip">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
          </svg>
          <span>{chipTime}</span>
        </div>

        {/* Theme toggle */}
        <button className="ib" onClick={onToggleTheme} aria-label="Toggle theme">
          {theme === 'light' ? (
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/>
            </svg>
          ) : theme === 'dark' ? (
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <circle cx="12" cy="12" r="5"/>
              <line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/>
              <line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/>
              <line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/>
              <line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/>
              <line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/>
              <line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/>
            </svg>
          ) : (
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <rect x="2" y="3" width="20" height="14" rx="2" ry="2"/>
              <line x1="8" y1="21" x2="16" y2="21"/>
              <line x1="12" y1="17" x2="12" y2="21"/>
            </svg>
          )}
        </button>

        {/* Notifications */}
        <button className="ib" aria-label="Notifications">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/>
            <path d="M13.73 21a2 2 0 01-3.46 0"/>
          </svg>
          {hasNotification && <span className="ndot"/>}
        </button>

        <div className="tb-div"/>

        {/* User */}
        <div className="tb-usr" aria-label="User info">
          <div className="tb-av">{staff.initials}</div>
          <div>
            <div className="tb-name">{staff.name}</div>
            <div className="tb-role">{staff.role}</div>
          </div>
        </div>
      </div>
    </header>
  );
};
