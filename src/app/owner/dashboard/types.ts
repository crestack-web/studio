...
export interface PurchaseOrder {
  ...
  items: Array<{ productId: string; productName: string; quantity: number; unit: string; unitCost: number; totalCost: number }>;
  subtotal: number;
  tax: number;
  shipping: number;
  total: number;
...
export interface StockReceipt {
  ...
  items: Array<{ productId: string; productName: string; quantity: number; unit: string; unitCost: number; totalCost: number; batchNumber?: string; expiryDate?: Date; location?: string }>;
  subtotal: number;
  tax: number;
  total: number;
  receivedDate: Date;
...
export interface SupplierPayment {
  ...
  paidByName: string;
  createdAt: Date;
  relatedPurchaseOrders?: string[];
  relatedStockReceipts?: string[];
}
...
export interface SupplierDashboard {
  ...
  topSupplierBySpend: { supplierId: string; supplierName: string; totalSpend: number } | null;
  monthlyPurchaseVolume: number;
  supplierCreditUtilization: number;
  supplierPaymentTrend: { month: string; payments: number; purchases: number }[];
}
...
export interface SupplierProfile {
  ...
  purchaseHistory: {
    ...
    productsSupplied: Array<{ productName: string; totalQuantity: number; totalAmount: number; lastSupplied: Date }>;
  };
  financials: {
    totalPurchases: number;
    totalPayments: number;
    ...
    creditUtilization: number;
    averagePaymentDays: number;
    paymentHistory: Array<{ month: string; amount: number; daysToPay: number }>;
  };
  ledger: SupplierLedgerTransaction[];
  analytics: {
    priceChanges: Array<{ productName: string; oldPrice: number; newPrice: number; changePercent: number; date: Date }>;
...
export interface PurchaseIntelligence {
  totalSpendBySupplier: Array<{ supplierId: string; supplierName: string; totalSpend: number; purchaseCount: number; percentageOfTotal: number }>;
  purchaseTrends: Array<{ month: string; totalPurchases: number; supplierCount: number; averageOrderValue: number }>;
  supplierPriceChanges: Array<{ supplierId: string; supplierName: string; productName: string; oldPrice: number; newPrice: number; changePercent: number; date: Date }>;
  supplierDependency: Array<{ supplierId: string; supplierName: string; dependencyScore: number; riskLevel: 'low' | 'medium' | 'high'; productsCount: number; percentageOfPurchases: number }>;
  productSourcing: Array<{ productId: string; productName: string; primarySupplier: string; alternativeSuppliers: string[]; lastPurchaseDate: Date; averageCost: number; costTrend: 'increasing' | 'decreasing' | 'stable' }>;
}
...
export interface CategorySupplierFeatures {
  ...
  customFields: Array<{ name: string; type: 'text' | 'number' | 'date' | 'select'; required: boolean; options?: string[] }>;
  analytics: string[];
  insights: string[];
}
