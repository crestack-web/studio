...
const DeadStockInsights: React.FC<DeadStockInsightsProps> = ({ products, onProductClick }) => {
  ...
  return (
    <div className="insight-card">
      <h2 className="insight-title">{t('insights.slowestMovers.title')}</h2>
      <p className="insight-sub">{t('insights.slowestMovers.subtitle')}</p>
            ...
            <div key={p.id} className="insight-item" onClick={() => onProductClick(p)}>
              <div className="insight-item-left">
                ...
                <div className="insight-item-detail">
                  {t('insights.lastSold')}: <span className="insight-item-date">{getDaysSinceLastSale(p)} {t('common.daysAgo')}</span>
                </div>
              </div>
              <div className="insight-item-right">
                <div className="insight-item-stock">{p.currentStock} {t('common.units')}</div>
